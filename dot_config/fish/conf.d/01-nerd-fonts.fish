...
# Fast check - use cache if available, otherwise assume installed on macOS
if test -f $__nerd_fonts_cache_file
    ...
    # Assume nerd fonts are installed and cache the result
    set -gx NERD_FONT_AVAILABLE 1
...
function check-nerd-fonts --description "Manually check for Nerd Fonts installation"
    ...
    # Use the slow but accurate system_profiler on macOS
